#############################################################################################################
# You got it\! Here's the complete Python script for fine-tuning an LLM with LoRA using your custom text file, 
...
print ("Loading torch Library")
import torch
print ("Loading peft Library")
from peft import LoraConfig, get_peft_model, prepare_model_for_kbit_training, TaskType, PeftModel
print ("Loading trl Library")
from trl import SFTTrainer
print ("Loading transformers Library")
from transformers import pipeline
print ("Loading Os Library")
import os
print ("Loading bitsandbytes Library")
import bitsandbytes 
print ("Loading tensorboard library")
import tensorboard
###############################################
...
#BASE_MODEL_ID = "meta-llama/Llama-2-7b-hf" # Make sure you have access to this model on Hugging Face
                                         # (e.g., Llama 2 requires approval) tinyllama
BASE_MODEL_ID = "TinyLlama/TinyLlama-1.1B-Chat-v1.0" # Make sure you have access to this model on Hugging Face
                                         # (e.g., Llama 2 requires approval) tinyllama                                         
...
# Target modules for Llama models. TinyLlama shares the same architecture as Llama 2.
...
# but the current ones are usually sufficient for initial fine-tuning
...
print ("STEP-1: Prepare Custom Data")
#############################################################################################################
...
print ("STEP-2: Choose a Base LLM and Tokenizer")
#############################################################################################################
...
print ("STEP-3: Setting up LoRA Configuration")
#############################################################################################################
...
print ("STEP-4: Prepare the Dataset for Training (Formatting Function)")
#############################################################################################################
...
#    return output_texts
    ...
    # This function receives a single example (dictionary)
    # and should return a single string
    instruction = example['instruction']
    response = example['response'] # Or 'output', 'completion', etc., based on your data

    # Construct the desired format as a single string
    text = f"### Instruction: {instruction}\n### Response: {response}"
    return text

print("--- 4. Data formatting function defined. ---")

print ("STEP-5: Define Training Arguments and SFTTrainer")
#############################################################################################################
...
training_arguments = TrainingArguments(
    ...
    fp16=True, # QLoRA uses bfloat16 for compute, so False here
    bf16=False, # Set to True if your GPU supports bfloat16 (recommended for Ampere+ GPUs) | If the GPU does not support make it false and fp16 true
    max_grad_norm=0.3,
    max_steps=-1,
    warmup_ratio=0.03,
...
print ("STEP-8: Load and Merge for Inference")
#############################################################################################################
